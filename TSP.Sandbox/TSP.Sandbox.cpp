...
int main(int argc, char* argv[])
{
	...
	auto Graph = TSP::SymmetricAdjacencyMatrix(matrix);
	Graph.Print();

	std::cout << std::endl;

	auto Graph2 = TSP::SymmetricAdjacencyMatrix(5);
	Graph2.AddEdge(0, 1, 10);
	Graph2.AddEdge(0, 2, 8);
	Graph2.AddEdge(0, 3, 9);
	Graph2.AddEdge(0, 4, 7);
	...
	auto Graph3 = TSP::AsymmetricAdjacencyMatrix(5);
	Graph3.AddEdge(0, 1, 10);
	Graph3.AddEdge(0, 2, 8);
	Graph3.AddEdge(0, 3, 9);
	Graph3.AddEdge(0, 4, 7);
	Graph3.AddEdge(1, 2, 10);
	Graph3.AddEdge(1, 3, 5);
	Graph3.AddEdge(1, 4, 6);
	Graph3.AddEdge(2, 3, 8);
	Graph3.AddEdge(2, 4, 9);
	Graph3.AddEdge(3, 4, 6);
	Graph3.Print();

	std::cout << std::endl;

	/*std::string filepath = "";
	auto Graph2 = TSP::ParseGraphFile<TSP::SymmetricAdjacencyMatrix>("C:\\Users\\jakub\\Documents\\Visual Studio 2017\\Projects\\PEA\\TSP\\Symmetric Instances\\51.txt");
	Graph2.Print();*/
	std::cin.get();
