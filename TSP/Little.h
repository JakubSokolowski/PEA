...
public:
	Little(Matrix<T> &m);
	void findTour();
	vector<int> getLastTour() { return lastTour; }    // Return the last found tour
	int getCost() { return reference; }               // Return the last found tour cost
	bool isOptimal() { return optimal; }              // Return whether the tour is optimal
};

// Return the minimum of a row in a matrix
template<typename T>
T Little<T>::getMinRow(Matrix<T> &m, int row, int ignoredCol) {
	int nbCol = m.getNbColumns();
	T min = infinity;
	for (int i = 1; i < nbCol; i++) {
		T currentValue = m.getValue(row, i);
		if (currentValue != infinity && i != ignoredCol) {
...
template<typename T>
T Little<T>::getMinCol(Matrix<T> &m, int col, int ignoredRow) {
...
template<typename T>
T Little<T>::reduceRow(Matrix<T> &m, int row) {
	int nbCol = m.getNbColumns();
	T min = getMinRow(m, row);
	for (int i = 1; i < nbCol; i++) {
		if (m.getValue(row, i) != infinity) {
...
template<typename T>
T Little<T>::reduceCol(Matrix<T> &m, int col) {
	int nbRow = m.getNbRows();
	T min = getMinCol(m, col);
	for (int i = 1; i < nbRow; i++) {
		if (m.getValue(i, col) != infinity) {
...
*/
template<typename T>
T Little<T>::reduceMatrix(Matrix<T> &m) {
	int nbRow = m.getNbRows();
	T minRowTotal = 0;
	for (int i = 1; i < nbRow; i++) {
...
*/
template<typename T>
T Little<T>::calculateRegret(Matrix<T> &m, pair<int, int> &path, pair<int, int> &pos) {
...
template<typename T>
void Little<T>::removeSubTour(Matrix<T> &m, int index, pair<int, int> &path) {
	int size = m.getNbRows();
	pair<int, int> pos;
	int founds = 0;
	...
	if (founds == 2) {
		m.setValue(pos.first, pos.second, infinity);
	}
}
...
template<class T> void Little<T>::findTour() {
	...
	while (!matrices.empty()) {     // Iterate till the stack is empty
		...
		* and until the current node is lower than the reference value */
		while (m.getNbRows() > 3 and tree[id].cost < reference) {
			...
			if (regretNode.cost < reference) {
				matrix.first = tree.size() - 1;
				matrix.second = m;
				matrix.second.setValue(pos.first, pos.second,
					infinity);      // Suppression case i, j pour une potentielle recherche ulterieur
				matrices.push(matrix);
			}
		...
		if (m.getNbRows() == 3) {
			if (normalNode.cost < reference) {
				addLastPath(m);
				reference = normalNode.cost;
				lastTour = orderPath(tree.size() - 1, 1);
	...
	optimal = true;   // Computing finished, the tour is thus optimal
...
template<class T> Little<T>::Little(Matrix<T> &m) {
	...
	initialMatrix = m;    // storage of the initial matrix
}
...
template<class T> void Little<T>::checkTourCost() {
	int cost = 0;
	int size = lastTour.size();
	for (int i = 0; i < size - 1; i++) {
		cost += initialMatrix.getValue(lastTour[i], lastTour[i + 1]);
	}
	cost += initialMatrix.getValue(lastTour.back(), lastTour.front());
